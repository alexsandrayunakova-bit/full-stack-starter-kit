...
import type { Category, Tag, Role } from "@/lib/types";
...
export default function NewToolPage() {
  ...
  const loadFormData = async () => {
    const [categoriesRes, tagsRes, rolesRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await api.post("/api/tools", submitData);

    if (response.data) {
      showToast("Инструментът е добавен успешно!", "success");
      router.push("/tools");
    } else {
      showToast(response.error || "Грешка при добавяне", "error");
    }
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-2">
              Попълнете информацията за новия инструмент
            </p>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                <Button type="submit" disabled={submitting} className="flex-1">
                  {submitting ? "Добавяне..." : "Добави инструмент"}
                </Button>
